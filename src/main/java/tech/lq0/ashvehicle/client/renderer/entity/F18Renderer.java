...
import tech.lq0.ashvehicle.client.model.F16Model;
import tech.lq0.ashvehicle.client.model.F18Model;
import tech.lq0.ashvehicle.entity.F16Entity;
import tech.lq0.ashvehicle.entity.F18Entity;

public class F18Renderer extends GeoEntityRenderer<F18Entity> {
    public F18Renderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new F18Model());
        this.shadowRadius = 0.0f;
    }
    public RenderType getRenderType(F18Entity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.entityTranslucent(this.getTextureLocation(animatable));
    }

    public void preRender(PoseStack poseStack, F18Entity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        float scale = 1.2F;
    ...
    public void render(F18Entity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        poseStack.pushPose();
    ...
    public void renderRecursively(PoseStack poseStack, F18Entity animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        ...
        if (name.equals("flapVR") || name.equals("flapVL")) {
        ...
        if (name.equals("gearF2")) {
            bone.setRotX(Mth.lerp(partialTick, animatable.gearRotO, (float)(Integer)animatable.getEntityData().get(MobileVehicleEntity.GEAR_ROT)) * ((float)Math.PI / 180F));
        }
        if (name.equals("gearF3")) {
        ...
        if (name.equals("gearR2")) {
            float degrees = Mth.lerp(partialTick, animatable.gearRotO,
                    (float)(Integer)animatable.getEntityData().get(MobileVehicleEntity.GEAR_ROT));

            bone.setRotY((degrees) * (-(float)Math.PI / 180F));
        }
        if (name.equals("gearL2")) {
            float degrees = Mth.lerp(partialTick, animatable.gearRotO,
                    (float)(Integer)animatable.getEntityData().get(MobileVehicleEntity.GEAR_ROT));
