...
public class ModEntities {
    ...
    private static <T extends Entity> EntityType.Builder<T> vehicle(
            EntityType.EntityFactory<T> factory, float w, float h
    ) {
    ...
    // ==== カテゴリ別ショートカット ====
    private static <T extends Entity> RegistryObject<EntityType<T>> aircraft(
            String name, EntityType.EntityFactory<T> factory
    ) {
        return reg(name, factory, AIRCRAFT_W, AIRCRAFT_H);
    }

    private static <T extends Entity> RegistryObject<EntityType<T>> tank(
            String name, EntityType.EntityFactory<T> factory
    ) {
        return reg(name, factory, TANK_W, TANK_H);
    }

    private static <T extends Entity> RegistryObject<EntityType<T>> largeTank(
            String name, EntityType.EntityFactory<T> factory
    ) {
        return reg(name, factory, TANK_W, TANK_H_LARGE);
    }

    private static <T extends Entity> RegistryObject<EntityType<T>> ifv(
            String name, EntityType.EntityFactory<T> factory
    ) {
        return reg(name, factory, IFV_W, IFV_H);
    }

    private static <T extends Entity> RegistryObject<EntityType<T>> ship(
            String name, EntityType.EntityFactory<T> factory
    ) {
    ...
    public static final RegistryObject<EntityType<pantsirS1Entity>> PANTSIR_S1 = tank("pantsir-s1", pantsirS1Entity::new);
