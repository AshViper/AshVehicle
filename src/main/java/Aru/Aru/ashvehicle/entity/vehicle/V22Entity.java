...
public class V22Entity extends ContainerMobileVehicleEntity implements GeoEntity, HelicopterEntity, WeaponVehicleEntity {
    ...
    public V22Entity(PlayMessages.SpawnEntity spawnEntity, Level level) {
        this((EntityType) ModEntities.V_22.get(), level);
    }
    ...
    public void travel() {
        ...
        if (this.getHealth() > 0.1F * this.getMaxHealth()) {
            ...
            if (passenger == null) {
                ...
                if (this.rightInputDown) {
                    ++this.holdTick;
                    this.entityData.set(DELTA_ROT, this.entityData.get(DELTA_ROT) - 2.0F * Math.min(this.holdTick, 7) * this.entityData.get(POWER));
                ...
                this.delta_y = Mth.clamp((this.onGround() ? 0.1F : 2.0F) * this.entityData.get(MOUSE_SPEED_X) * this.entityData.get(PROPELLER_ROT), -10.0F, 10.0F);
            ...
            if (this.level() instanceof ServerLevel) {
                if (this.getEnergy() > 0) {
                    if (this.entityData.get(GEAR_ROT) > 0) {
                        // — 戦闘機モード —
                        if (this.forwardInputDown) {
                            this.entityData.set(POWER, Math.min(this.entityData.get(POWER) + 7.0E-4F * Math.min(this.holdPowerTick, 10), 0.12F));
                        }
                        if (this.backInputDown) {
                            this.entityData.set(POWER, Math.max(this.entityData.get(POWER) - 0.001F * Math.min(this.holdPowerTick, 5), this.onGround() ? 0.0F : 0.025F));
                        }
                        if (!this.onGround()) {
                            if (this.rightInputDown) {
                        ...
                        // — ヘリモード —
                        boolean up = this.forwardInputDown;   // W 上昇
                        boolean down = this.backInputDown;    // S 下降
                        ...
                        if (!up && !down && this.engineStartOver) {
                            if (this.getDeltaMovement().y() < 0.0F) {
                    ...
                    // プロペラ回転更新
                    this.setPropellerRot(
                            this.getPropellerRot() + 30.0F * this.entityData.get(POWER)
                    );

                    // ギア（ティルト角）制御
                    if (this.upInputDown) {
                        this.upInputDown = false;
                        if (this.entityData.get(GEAR_ROT) == 0 && !this.onGround()) {
                            this.entityData.set(GEAR_UP, true);
                        } else if (this.entityData.get(GEAR_ROT) == 85) {
                    ...
                    if (this.entityData.get(GEAR_UP)) {
                        this.entityData.set(GEAR_ROT, Math.min(this.entityData.get(GEAR_ROT) + 5, 85));
                    } else {
                        this.entityData.set(GEAR_ROT, Math.max(this.entityData.get(GEAR_ROT) - 5, 0));
                    }
                    ...
                    this.entityData.set(
                            POWER,
                            Math.max(this.entityData.get(POWER) - 1.0E-4F, 0.0F)
                    );
                    this.forwardInputDown = false;
                    this.backInputDown = false;
            ...
            if (this.getEnergy() > 0) {
                ...
                this.setDeltaMovement(this.getDeltaMovement().add(0.0, -this.destroyRot * 0.004, 0.0));
            }
        }
        ...
        this.entityData.set(
                PROPELLER_ROT,
                Mth.lerp(0.18F, this.entityData.get(PROPELLER_ROT), this.entityData.get(POWER))
        );
        this.setPropellerRot(
                this.getPropellerRot() + 30.0F * this.entityData.get(PROPELLER_ROT)
        );
        ...
        // ─── オスプレイ用：垂直⇔水平推力補間 ───
        float tiltRatio = Mth.clamp(this.entityData.get(GEAR_ROT) / 85.0F, 0.0F, 1.0F);
        Vec3 vertical = new Vec3(0.0, 1.0, 0.0);
        Vec3 forward = this.getForward().normalize();
        Vec3 thrustDir = vertical.scale(1.0F - tiltRatio).add(forward.scale(tiltRatio)).normalize();
        float thrustPower = Mth.lerp(tiltRatio, 1.0F, 2.5F);

        this.setDeltaMovement(
                this.getDeltaMovement().add(thrustDir.scale(this.entityData.get(POWER) * thrustPower))
        );

        // ② 揚力（リフト）計算
        Matrix4f transform = this.getVehicleTransform(1.0F);
        Vector4f p0 = this.transformPosition(transform, 0,0,0);
        Vector4f p1 = this.transformPosition(transform, 0,1,0);
        Vec3 liftDir = new Vec3(p0.x,p0.y,p0.z).vectorTo(new Vec3(p1.x,p1.y,p1.z));
        double speedFactor = this.getDeltaMovement().dot(this.getViewVector(1.0F)) * 0.022;
        double flapAngle = (this.getFlap1LRot() + this.getFlap1RRot() + this.getFlap1L2Rot()+ this.getFlap1R2Rot()) / 4.0;
        double liftCoeff = 1.0 + Math.sin((this.onGround() ? 25.0 : flapAngle + 25.0) * Math.PI/180);
        this.setDeltaMovement(this.getDeltaMovement().add(liftDir.scale(speedFactor * liftCoeff)));

        // ③ 追加スラスター推力
        this.setDeltaMovement(this.getDeltaMovement().add(this.getViewVector(1.0F).scale(0.2 * this.entityData.get(POWER))));

        // エンジン起動判定
        if (this.entityData.get(POWER) > 0.04F) {
            this.engineStartOver = true;
        }
        if (this.entityData.get(POWER) < 4e-4F) {
            this.engineStart = false;
            this.engineStartOver = false;
        }
    }
    ...
    public void positionRider(@NotNull Entity passenger, @NotNull Entity.@NotNull MoveFunction callback) {
        if (this.hasPassenger(passenger)) {
            ...
            if (i == 0) {
                ...
                Vector4f worldPosition = this.transformPosition(transform, 0.7F, 0.5F, 5.5F);
                passenger.setPos((double)worldPosition.x, (double)worldPosition.y, (double)worldPosition.z);
                callback.accept(passenger, (double)worldPosition.x, (double)worldPosition.y, (double)worldPosition.z);
            } else if (i == 3) {
                Vector4f worldPosition = this.transformPosition(transform, -0.7F, 0.5F, 5.5F);
    ...
    static {
        PROPELLER_ROT = SynchedEntityData.defineId(V22Entity.class, EntityDataSerializers.FLOAT);
        LOADED_ROCKET = SynchedEntityData.defineId(V22Entity.class, EntityDataSerializers.INT);
    }
}