...
import tech.lq0.ashvehicle.client.model.SU33Model;
import tech.lq0.ashvehicle.entity.F4Entity;
import tech.lq0.ashvehicle.entity.SU33Entity;

public class SU33Renderer extends GeoEntityRenderer<SU33Entity> {
    public SU33Renderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new SU33Model());
        this.shadowRadius = 0.0f;
    }
    public RenderType getRenderType(SU33Entity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.entityTranslucent(this.getTextureLocation(animatable));
    }

    public void preRender(PoseStack poseStack, SU33Entity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        float scale = 1.0F;
    ...
    public void render(SU33Entity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        poseStack.pushPose();
    ...
    public void renderRecursively(PoseStack poseStack, SU33Entity animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        ...
        if (name.equals("FlapLB2")) {
            bone.setRotX(Mth.lerp(partialTick, animatable.flap2LRot, animatable.getFlap2LRot()) * (-(float)Math.PI / 180F));
        }

        if (name.equals("FlapRB2")) {
            bone.setRotX(Mth.lerp(partialTick, animatable.flap2RRot, animatable.getFlap2RRot()) * (-(float)Math.PI / 180F));
        }
