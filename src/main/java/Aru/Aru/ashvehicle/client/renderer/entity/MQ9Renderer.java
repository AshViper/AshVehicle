...
import Aru.Aru.ashvehicle.client.model.MQ9Model;
import Aru.Aru.ashvehicle.entity.MQ9Entity;

public class MQ9Renderer extends GeoEntityRenderer<MQ9Entity> {
    public MQ9Renderer(EntityRendererProvider.Context renderManager) {
        super(renderManager, new MQ9Model());
        this.shadowRadius = 0.0f;
    }
    public RenderType getRenderType(MQ9Entity animatable, ResourceLocation texture, MultiBufferSource bufferSource, float partialTick) {
        return RenderType.entityTranslucent(this.getTextureLocation(animatable));
    }

    public void preRender(PoseStack poseStack, MQ9Entity entity, BakedGeoModel model, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        float scale = 3.5F;
    ...
    public void render(MQ9Entity entityIn, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        poseStack.pushPose();
    ...
    public void renderRecursively(PoseStack poseStack, MQ9Entity animatable, GeoBone bone, RenderType renderType, MultiBufferSource bufferSource, VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
